...
router.get('/', function (req, res, next) {
  req.getConnection(function (err, connection) {
    if (connection) {
      connection.query('SELECT * FROM empresa ORDER BY id_empresa; SELECT e.nome_empresa as nome_empresa, tipo_veiculo, qtd_cobertas, qtd_descobertas FROM parametros_empresa p INNER JOIN empresa e on p.id_empresa=e.id_empresa ORDER BY p.id_empresa;', function (err, rows) {
        if (rows) {
